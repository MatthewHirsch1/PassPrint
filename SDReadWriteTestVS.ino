...
void loop() {
  if (Serial.available()) {
    ...
    if (data == "Add Credential") {
      addOneCredential("test.txt", "test.com:bob123@test.com:password:");
    }
    ...
    if (data == "Saving Credential") {
      String account = Serial.readStringUntil(':');
      String uname = Serial.readStringUntil(':');
      String pwd = Serial.readStringUntil(':');
      addOneCredential(testFile, account + ":" + uname + ":" + pwd + ":");
    }
